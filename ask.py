...
# Entity resolution (name->ticker, TICKER=weight, ticker validation) was lifted into
# the shared spine so it's reusable + unit-tested independently of this front door.
from imdata.entities import (                # noqa: E402
    valid_ticker, match_company_name, extract_tickers, extract_positions,
    ensure_universe as _load_universe)
...
def classify(text: str, keyword_first: bool = False) -> tuple:
    """Route a request to a system. The model is the PRIMARY router now; the keyword
    heuristic is SECONDARY — a fallback for when no model is available or the model
    abstains. Pass ``keyword_first`` (the --keyword flag) to force the deterministic
    keyword path for scripted / offline runs."""
    if keyword_first:
        sysname, score = heuristic_system(text)
        if score >= 3:
            return sysname, "keyword"
    # Primary: let the model route (qwen → sonnet → opus per the ladder).
    ...
    if not res.get("_needs_model") and res.get("system") in SYSTEMS:
        return res["system"], ("model:" + res.get("_route", "?"))
    # Secondary: deterministic keyword heuristic (model unavailable or abstained).
    sysname, score = heuristic_system(text)
    if score > 0:
        return sysname, "keyword(fallback)"
    return "idea-sourcing", "fallback"
...
def main():
    ...
    ap.add_argument("--keyword", action="store_true",
                    help="force the deterministic keyword router (offline / scripted; "
                         "the model router is the default primary path)")
    args = ap.parse_args()
    ...
    if args.system:
        ...
        system, why = classify(text, keyword_first=args.keyword)

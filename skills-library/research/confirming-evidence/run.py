"""confirming-evidence: marshal tagged, cited EVIDENCE FOR a stock. Hybrid model skill.
...
SCHEMA = {
    ...
    "properties": {
        ...
        "evidence": {
            "type": "array",
            "description": "Tagged, cited evidence objects supporting the case FOR the stock. "
                           "Each item is a single piece of evidence, never a thesis or a call.",
            "items": {
                "type": "object",
                "properties": {
...
SYSTEM = (
    ...
    "EVIDENCE ONLY for the CONFIRMING side — the case FOR the stock: durable strengths, "
    "positive drivers, and supporting facts. You do NOT write a thesis, a conclusion, or a "
    "recommendation, and you NEVER use buy/sell/recommend/attractive/overweight/underweight or "
    "any trade-direction language. Emit a list of tagged, cited evidence objects. Each object's "
    "`tag` is the fixed string \"confirming\"; each `claim` is one factual point; each `citation` "
    "references a provided figure (a revenue/net-income value with its period, the consensus "
    "target/PE/growth, or the last price) or a filing fact; `dimension` is one of "
    ...
    "to rebut, you MAY address those specific points, but still only as confirming evidence "
    "(facts that counter them), never as argument, thesis, or a call."
)
...
def main(args):
    ...
    parts.append(
        "Marshal the CONFIRMING evidence (the case FOR this company): durable strengths and "
        "positive drivers, each grounded in a provided figure or filing fact. Use the "
        "revenue/net-income trend to evidence growth and profitability; use the consensus "
        "target/PE/growth and last price to evidence valuation support. Produce a list of "
        "tagged, cited evidence objects (tag must be \"confirming\"); set ticker, company, and "
    ...
    if rebut:
        parts.append(
            "\nThe DISCONFIRMING side raised these specific points. You MAY address them, but "
            "ONLY by supplying confirming evidence (facts that bear on them) — never as argument, "
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(
        description="Marshal tagged, cited confirming (case-FOR) evidence for a stock — evidence only, no thesis.")

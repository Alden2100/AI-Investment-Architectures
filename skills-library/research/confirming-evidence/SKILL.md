...
version: 1.0.0
description: Marshals tagged, cited EVIDENCE FOR a stock — durable strengths and positive
  drivers grounded in revenue/net-income trends, consensus estimates, target, and last price.
  Emits evidence objects only, never a thesis or buy/sell recommendation. Use when an
  adversarial idea-sourcing debate needs the confirming (case-FOR) side, "evidence for
  <ticker>", "what supports owning this", or the bull side of a structured debate.
---
# Confirming Evidence
...
model then marshals the CONFIRMING side: a list of tagged, cited evidence objects making the case
FOR the company. This is Stage 6 of idea-sourcing v2 (the adversarial debate pair). It produces
EVIDENCE ONLY — never a thesis, conclusion, or recommendation, and no buy/sell language.

Each evidence object is `{tag:"confirming", claim, citation, dimension, confidence}`, where
...
python run.py --ticker AAPL --round 2 --rebut '[{"tag":"disconfirming","claim":"...","citation":"...","dimension":"growth"}]'
```

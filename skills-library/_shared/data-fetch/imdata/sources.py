...
SOURCES: dict = {
    ...
    "form4":           {"name": "SEC Form 4 insider (XML)",     "module": "ownership", "tier": PUBLIC, "requires_key": False, "status": "planned", "attribution": "U.S. SEC EDGAR"},
    "sched_13dg":      {"name": "SEC SC 13D/13G ownership",     "module": "ownership", "tier": PUBLIC, "requires_key": False, "status": "planned", "attribution": "U.S. SEC EDGAR"},
    ...
    "polygon":         {"name": "Polygon.io (free)",  "module": "prices", "tier": KEY_EVAL, "requires_key": True, "env": "POLYGON_API_KEY", "status": "planned", "attribution": "Polygon.io"},
    "alpha_vantage":   {"name": "Alpha Vantage",      "module": "prices", "tier": KEY_EVAL, "requires_key": True, "env": "ALPHAVANTAGE_API_KEY", "status": "planned", "attribution": "Alpha Vantage"},
    ...
    "cboe_vix":        {"name": "CBOE VIX (CSV)",     "module": "volatility", "tier": PUBLIC, "requires_key": False, "status": "planned", "attribution": "Cboe Global Markets"},
    "finra_short":     {"name": "FINRA short interest", "module": "ownership", "tier": PUBLIC, "requires_key": False, "status": "planned", "attribution": "FINRA"},
    ...
    "damodaran":       {"name": "Damodaran datasets (ERP/beta/WACC)", "module": "valinputs", "tier": PUBLIC, "requires_key": False, "status": "building", "attribution": "Aswath Damodaran, NYU Stern (pages.stern.nyu.edu)"},
    "segments":        {"name": "Segment KPIs (XBRL dimensional)", "module": "segments", "tier": PUBLIC, "requires_key": False, "status": "planned", "attribution": "U.S. SEC EDGAR"},
    ...
    "fmp":             {"name": "Financial Modeling Prep",  "module": "fmp", "tier": KEY_EVAL, "requires_key": True, "env": "FMP_API_KEY", "status": "planned", "attribution": "Financial Modeling Prep"},
    "simfin":          {"name": "SimFin standardized stmts", "module": "fmp", "tier": KEY_EVAL, "requires_key": True, "env": "SIMFIN_API_KEY", "status": "planned", "attribution": "SimFin"},
    ...
    # --- Macro ---------------------------------------------------------------
    "treasury":        {"name": "US Treasury yield curve",  "module": "macro", "tier": PUBLIC, "requires_key": False, "status": "building", "attribution": "U.S. Department of the Treasury (public domain)"},
    "fred":            {"name": "FRED (St. Louis Fed)",     "module": "macro", "tier": PUBLIC, "requires_key": True, "env": "FRED_API_KEY", "status": "building", "attribution": "Federal Reserve Bank of St. Louis (FRED); avoid 3rd-party S&P series"},
    ...
    "world_bank":      {"name": "World Bank",               "module": "macro", "tier": PUBLIC, "requires_key": False, "status": "planned", "attribution": "World Bank Open Data"},
    "ecb":             {"name": "ECB SDW",                  "module": "macro", "tier": PUBLIC, "requires_key": False, "status": "planned", "attribution": "European Central Bank"},
    "cftc_cot":        {"name": "CFTC Commitments of Traders", "module": "macro", "tier": PUBLIC, "requires_key": False, "status": "planned", "attribution": "U.S. CFTC"},
    ...
    "newsapi":         {"name": "NewsAPI",                  "module": "news", "tier": KEY_EVAL, "requires_key": True, "env": "NEWSAPI_KEY", "status": "planned", "attribution": "NewsAPI (content copyrighted)"},
    "gnews":           {"name": "GNews",                    "module": "news", "tier": KEY_EVAL, "requires_key": True, "env": "GNEWS_KEY", "status": "planned", "attribution": "GNews"},
    "reddit":          {"name": "Reddit (PRAW)",            "module": "altdata", "tier": KEY_EVAL, "requires_key": True, "env": "REDDIT_CLIENT_ID", "status": "planned", "attribution": "Reddit API"},
    "pytrends":        {"name": "Google Trends (pytrends)", "module": "altdata", "tier": KEYLESS, "requires_key": False, "status": "planned", "attribution": "Google Trends (unofficial)"},
    ...
    # --- Governance ----------------------------------------------------------
    "ofac_sdn":        {"name": "OFAC SDN list",            "module": "sanctions", "tier": PUBLIC, "requires_key": False, "status": "planned", "attribution": "U.S. Treasury OFAC (public)"},
    "eu_sanctions":    {"name": "EU consolidated sanctions", "module": "sanctions", "tier": PUBLIC, "requires_key": False, "status": "planned", "attribution": "European Union (public)"},
    "un_sanctions":    {"name": "UN Security Council list",  "module": "sanctions", "tier": PUBLIC, "requires_key": False, "status": "planned", "attribution": "United Nations (public)"},
    "uk_sanctions":    {"name": "UK OFSI sanctions",         "module": "sanctions", "tier": PUBLIC, "requires_key": False, "status": "planned", "attribution": "UK OFSI (public)"},
    "opensanctions":   {"name": "OpenSanctions aggregated",  "module": "sanctions", "tier": KEY_NC, "requires_key": True, "env": "OPENSANCTIONS_API_KEY", "status": "planned", "attribution": "OpenSanctions (non-commercial license)"},
}

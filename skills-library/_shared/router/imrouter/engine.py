...
the local qwen3.5:9b (cheap, high-volume) or the Claude API (heavy reasoning).
...
#   claude -> Claude API (heavy reasoning / synthesis / final drafting / judgment)
# --------------------------------------------------------------------------- #
DEFAULT_POLICY = {
    "default": "claude",
...
def route(prompt: str, *, task: str, system: str = "", schema: Optional[dict] = None,
    ...
    base = {"task": task, "desired": desired, "chosen": chosen, "fell_back": fell_back}

    if chosen == "none":
        _log({**base, "result": "needs_model"})
        return {"_needs_model": True, "system": system, "prompt": prompt,
                "schema": schema, "_route": "none"}
    ...
    try:
        if chosen == "local":
            ...
            out.setdefault("_source", "api")
            out["_route"] = "claude"
            out["_model"] = model or claude_client.DEFAULT_MODEL
        _log({**base, "model": out.get("_model"), "result": "ok"})
        return out
    except Exception as e:  # noqa: BLE001 — degrade to needs_model, never crash a skill
        _log({**base, "result": "error", "error": f"{type(e).__name__}: {e}"})
        return {"_needs_model": True, "system": system, "prompt": prompt,
                "schema": schema, "_route": chosen, "_error": str(e)}

"""comps-refresher: force-refresh prices/financials and rebuild a comps table for a saved peer set. Deterministic."""
import argparse
import os
import statistics
import sys
...
def metrics(ticker):
    # Force fresh data before reading.
    edgar.refresh_facts(ticker)
    prices.refresh_prices(ticker, force=True)
...
def main(args):
    ...
    summary = (f"Refreshed comps for {', '.join(out['peers'])}: median EV/EBITDA "
               f"{med['ev_ebitda']:.1f}x, P/E {med['pe']:.1f}x, P/S {med['ps']:.1f}x."
               if all(med.values()) else
               f"Refreshed comps for {', '.join(out['peers'])}.")
    ...
    summary += " Data was force-refreshed (financials + prices) before computing."
    out["summary"] = summary
    return out


if __name__ == "__main__":
    p = argparse.ArgumentParser(description="Refresh and rebuild a comps table.")
